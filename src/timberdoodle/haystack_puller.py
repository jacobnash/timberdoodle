...
def pull_equip(client: HaystackClient, store, source_id: str, equip_filter: str) -> None:
...
def pull_points(client: HaystackClient, store, ts_conn, source_id: str, point_filter: str, backfill_days: float) -> None:
    now = time.time()
...
def pull_once(client: HaystackClient, store, ts_conn, source_id: str, point_filter: str, equip_filter: str, backfill_days: float) -> None:
    with tracer.start_as_current_span("haystack_puller.pull_once") as span:
        span.set_attribute("source_id", source_id)
        pull_equip(client, store, source_id, equip_filter)
        pull_points(client, store, ts_conn, source_id, point_filter, backfill_days)
